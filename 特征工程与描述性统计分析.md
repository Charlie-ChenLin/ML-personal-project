...
在建模数据集（`q1_*.csv`）的特征列已统一做 `shift(1)` 的前提下，新增特征只基于这些“已滞后”的序列做派生，不会使用未来信息。
...
- `figures/raw_<因子>.png`：每张表的原始时序 + 月度平滑曲线 + 分布直方图
...
- `monthly_coverage_heatmap.png`：每个因子月度可用性（1/0）
- `monthly_coverage_counts.png`：每个因子可用月份数量
- `lag_corr_spearman.csv` / `lag_corr_heatmap.png`：因子均值与 PP 价格的滞后相关（factor(t-lag) vs price(t)）

### C. 建模数据集（已 shift(1)，严格时间序列）

对每个 `--dataset` 生成一套结果（`outputs/eda/<dataset_stem>/`）：

- `target_series.png`：`y` 与 `y_return` 时序图 + 测试窗口阴影
- `seasonality_boxplots.png`：按月份的季节性箱线图（y 与 y_return）
- `missingness_heatmap.png`：按“因子组”统计的逐月缺失率热力图
- `top_corr_with_y.png` + `corr_with_y.csv`：与 y 的相关性（特征已是滞后特征）
- `acf_y.png` / `acf_y_return.png`：自相关（ACF）
- `adf_tests.csv`：ADF 平稳性检验（y 与 y_return）
- `seasonal_decompose_y.png`：季节分解（数据足够长时生成）

### 一键运行

```bash
# 默认会尝试分析 outputs/datasets/q1_long.csv 与 outputs/datasets/q1_with_futures.csv（若存在）
conda run -n lchen python scripts/run_q1_eda.py

# 指定数据集（可重复 --dataset）
conda run -n lchen python scripts/run_q1_eda.py --dataset outputs/datasets/q1_long.csv

# 在 EDA 前对数据集额外应用一次特征工程（用于对比）
conda run -n lchen python scripts/run_q1_eda.py --dataset outputs/datasets/q1_long.csv --engineer-features
```
