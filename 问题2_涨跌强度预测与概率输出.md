...
实现位置：`pp_forecast/dataset.py:1`

> 你可以通过 `scripts/build_pp_dataset.py` 的 `--strong-threshold/--flat-threshold` 调整阈值（写报告时建议说明阈值依据，并做敏感性对比）。
...
- `outputs/metrics/<dataset_stem>/pp_test_predictions.csv`（含 `return_pred` 与 `strength_pred`）
...
- `outputs/metrics/<dataset_stem>/pp_strength_test_predictions.csv`
  - `strength_true`：真实标签
  - `strength_pred`：预测标签
...
- `bagging_tree_clf`：Bagging(DecisionTree)
- `svc_rbf`：SVC（开启 `probability=True`）
- `nb`：Gaussian Naive Bayes
- `knn_clf`：KNN（样本很少时可能失败，已记录到 failures）

参数会写到：`outputs/metrics/<dataset_stem>/pp_strength_params_<model>.json`。

另外提供集成（soft-voting）输出：

- `ensemble_proba_mean`：各模型概率简单平均
- `ensemble_proba_topk_cv_weighted`：按训练集 TimeSeriesSplit 的 macro-F1 做 Top-k 加权平均

对应文件：

- 训练集 CV：`outputs/metrics/<dataset_stem>/pp_strength_model_cv.csv`
- 集成预测与各档概率：`outputs/metrics/<dataset_stem>/pp_strength_ensemble_test_predictions.csv`
...
以下为默认测试窗（2021-01~2021-07）的一次结果摘要；完整对比见 `outputs/metrics/<dataset_stem>/pp_strength_model_metrics.csv`。

|数据集|说明|最佳模型（按 F1_macro）|F1_macro|Accuracy|Precision_macro|Recall_macro|
|---|---|---:|---:|---:|---:|---:|
|`pp_base`|不含期货|`rf_clf`|0.444|0.571|0.367|0.583|
|`pp_base_engineered`|不含期货 + 特征工程|`ada_clf`|0.489|0.714|0.556|0.500|
|`pp_with_futures`|含期货（restrict）|`logreg`|0.242|0.571|0.190|0.333|
|`pp_with_futures_engineered`|含期货 + 特征工程（restrict）|`ada_clf`|0.413|0.571|0.389|0.500|
...
conda run -n lchen python scripts/run_pp_models.py --dataset outputs/datasets/pp_base_engineered.csv
```
